...
findusbs () {
echo "Scanning for connected USB and SD storage devices..."
echo "------------------------------------------------"

# Mapfile reads the list of USB/SD devices into an array.
# $4=="usb" catches standard USB drives and USB SD-card adapters.
# $1 ~ /^mmcblk/ catches natively connected internal SD cards.
mapfile -t REMOVABLE_DRIVES < <(lsblk -b -do NAME,SIZE,MODEL,TRAN | awk '($4=="usb" || $1 ~ /^mmcblk/) {
    size_mb = $2 / 1048576;
    printf "/dev/%s (%.2f MB - %s)\n", $1, size_mb, $3
}')

# Check if any USB/SD devices were found
if [ ${#REMOVABLE_DRIVES[@]} -eq 0 ]; then
    echo "No USB or SD storage devices detected."
    exit 1
fi
...
for i in "${!REMOVABLE_DRIVES[@]}"; do
    echo "[$((i+1))] ${REMOVABLE_DRIVES[$i]}"
done
echo "------------------------------------------------"

# Prompt the user to make a selection
while true; do
    read -p "Select a device by entering the number (1-${#REMOVABLE_DRIVES[@]}): " CHOICE
    
    # Check if input is a valid integer and within the array range
    if [[ "$CHOICE" =~ ^[0-9]+$ ]] && [ "$CHOICE" -ge 1 ] && [ "$CHOICE" -le "${#REMOVABLE_DRIVES[@]}" ]; then
        INDEX=$((CHOICE - 1))
        DEVICE=$(echo "${REMOVABLE_DRIVES[$INDEX]}" | awk '{print $1}')
        
        # Extract the size, strip the parenthesis, and round to a whole integer
        RAW_SIZE=$(echo "${REMOVABLE_DRIVES[$INDEX]}" | awk '{print $2}' | tr -d '(')
...
SWAP_MB=$(awk -v size="$SIZE" 'BEGIN { printf "%.0f", ((size * 128) / 1984) - 1024 }')

# Rootfs takes the remainder to ensure everything adds up perfectly
ROOTFS_MB=$((SIZE - BOOTFS_MB - SWAP_MB - "1024"))
...
bootfs () {
    # Wipes drive to partition correctly
    sudo sfdisk --delete $DEVICE
    # Maxes the bootfs_mb partition to 2GB
    if [ "$BOOTFS_MB" -gt 2048 ]; then
    BOOTFS_MB=2048
    fi
    ...
    if [[ "$DEVICE" == /dev/mmcblk* ]]; then
        sudo /sbin/mkfs.vfat -n boot "${DEVICE}p1"
    else
...
rootfs () {
    ...
    if [[ "$DEVICE" == /dev/mmcblk* ]]; then
        sudo /sbin/mkfs.ext2 -L aurum "${DEVICE}p2"
    else
...
mountusb () {
    # creates mount points & mounts partitions
    sudo mkdir -p "$bootfs"
    sudo mkdir -p "$rootfs"
    sudo mkdir -p ./mount/proc

    if [[ "$DEVICE" == /dev/mmcblk* ]]; then
        sudo mount -f "${DEVICE}p1" "$bootfs"
        sudo mount -f "${DEVICE}p2" "$rootfs"
        sudo swapon "${DEVICE}p3"
    else
        sudo mount -f "${DEVICE}1" "$bootfs"
        sudo mount -f "${DEVICE}2" "$rootfs"
        sudo swapon "${DEVICE}3"
    fi
